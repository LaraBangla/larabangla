    ...
    <div class="container mx-auto bg-gray-100 rounded-lg mt-10">
        <div class="p-5">
            <form action="{{ route('admin.store.division') }}" method="post">
                ...
                <input type="text" name="name" value="{{ old('name') }}" id="name" class="w-1/3 py-3" required/>
                @error('name')
                ...
                <input type="text" name="slug" value="{{ old('slug') }}" id="slug" class="w-1/3 py-3" required />
                @error('slug')
                <div class=" text-red-500 mt-1 font-medium">{{ $message }}</div>
                @enderror
                </div>

                <button type="submit" class="mt-10 px-5 py-3 bg-gray-200 uppercase rounded-lg font-bold hover:bg-gray-300  ">ADD Devision</button>
            </form>
        </div>
    </div>
</section>
